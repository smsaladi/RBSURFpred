...
--- apply window of size 9 and prepare final input for ASA prediction by RBSURpred
--- This is input for exact method, so contains, bias + (feature * 9) columns
*/
	...
	while (lineCount <= seqLength)
	{
		...
		while (window_cover < WINDOW_SIZE)
		{
			if (start < 1)
			{
				...
				//printf("\nstart = %d,DUMb = %d",start,feature_count);
				window_cover++;
				start++;
			}
