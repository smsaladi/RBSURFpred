...
int main(int argc, char *argv[])
{
	...
	fgets(pssmline, sizeof pssmline, pssm);				// skip header
	//====================================================================================================================================
	///Check the spacing and set the flag accordingly for future use
	int pssm_flag = -1;
	int p_counter=0;
    // Take pssm value

    fgets(pssmline, sizeof pssmline, pssm);
    char *pssm_val_str = substring(pssmline, 11, 79);
    p_counter = 0;
    char *tpssmm;
    tpssmm = strtok(pssm_val_str, " ");

    while (tpssmm != NULL)
    {
        //cout<<atoi(tpssm)<<endl;						// Collect each pssm (normalized)
        tpssmm = strtok(NULL, " ");
        p_counter++;
    }

    if(p_counter > 20) pssm_flag = 0;/// use (11,60)
    else pssm_flag = 1;///use (11,79)

    fclose(pssm);


    ///Re open the pssm file
	...
	pssm = fopen(pssmFileName, "r");							// open fasta file and read sequence
	if (pssm == NULL) {
		fprintf(stderr, "Can't open pssm file!\n");
		exit(1);
	}
	...
	while (!feof(pssm))
	{
		...
		if (strlen(pssmLine) > 60)
		{
			seqLength++;
			char *pssmSub;
			if(pssm_flag == 1)pssmSub = substring(pssmLine, 11, 79);
			else if(pssm_flag == 0)pssmSub = substring(pssmLine, 10, 60);
			//fprintf(nfp, "%s\n", pssmSub);

			for (int j = 0; j < 20; j++)
			{
				char* t;
				if(pssm_flag == 1)t = substring(pssmSub, (j * 4) + 1, 4);
				else if(pssm_flag == 0)t = substring(pssmSub, (j * 3) + 1, 3);
	...
	for (int i = 0; i < 20; i++)										// Iterate for 20 possible amino acid
	{
		for (int j = 0; j < 20; j++)									// Iterate for 20 possible amino acids (form pair)
		{
			for (int k = 0; k < seqLength - 1; k++)						// Iterate for length of the sequence
			{
				...
				char *pssmSub;
				if(pssm_flag == 1)pssmSub = substring(pssmLine, 11, 79);			// Parse PSSM information line
				else if(pssm_flag == 0)pssmSub = substring(pssmLine, 10, 60);			// Parse PSSM information line

                char *pssmSub1;
				if(pssm_flag == 1)pssmSub1 = substring(pssmLine1, 11, 79);			// Parse PSSM information line
				else if(pssm_flag == 0)pssmSub1 = substring(pssmLine1, 10, 60);			// Parse PSSM information line

				char* t;
				if(pssm_flag == 1)t = substring(pssmSub, (i * 4) + 1, 4);					// Parse PSSM value
				else if(pssm_flag == 0)t = substring(pssmSub, (i * 3) + 1, 3);					// Parse PSSM value
				...
				char* t1;
				if(pssm_flag == 1)t1 = substring(pssmSub1, (j * 4) + 1, 4);					// Parse PSSM value
				else if(pssm_flag == 0)t1 = substring(pssmSub1, (j * 3) + 1, 3);					// Parse PSSM value
